...
Каждый из 499 шагов WALKTHROUGH начинается с выделенного жирным пояснения
**«Зачем: …»**. Оно объясняет назначение действия; далее приведены команды,
ожидаемый вывод и уточнения. Тот же формат используется для этапов README,
включая замену ISC DHCP на Kea. Наличие пояснений проверяет
`scripts/validate-course-content.py`; соответствие смысла командам проверяется при ревью.

1. **Зачем: изолировать эксперимент и подготовить зависимости выбранной ОС.** Выберите отдельную ВМ/роль и установите пакеты своей ОС по README.
2. **Зачем: использовать воспроизводимые материалы с известным источником.** Получите файлы из этого репозитория, архивы — по указанным официальным URL.
3. **Зачем: обеспечить доверие и аутентификацию без передачи лишних секретов.** Создайте/получите только необходимые ключи согласно PREPARATION.
4. **Зачем: получить целостную конфигурацию для своей сети без неявных недостающих блоков.** Скопируйте **полный** файл из выбранного варианта, замените адреса своей схемы.
5. **Зачем: выявить ошибки до применения и подтвердить реальное поведение сервиса.** Проверьте синтаксис штатной программой, примените и выполните проверки WALKTHROUGH.
6. **Зачем: зафиксировать доказательства результата без публикации секретов.** Сохраните отчет: версии, effective config без секретов, positive/negative результат.
