...
**Зачем: До проверки приложений согласуйте FQDN, время и записи A, PTR и MX: ошибки этих зависимостей проявляются сразу в нескольких почтовых компонентах. Получите полное имя сервера, которое используется DNS, TLS и почтовыми протоколами.**
...
**Зачем: Проверьте часовой пояс и синхронизацию системных часов; это необходимо для сертификатов, TOTP и сопоставления журналов.**
...
**Зачем: Запросите одну конкретную DNS-запись; сервер, имя и тип ответа должны соответствовать текущему сетевому сценарию.**
...
**Зачем: Проверяем PTR почтового адреса, чтобы обратное разрешение согласовывалось с именем почтового сервера.**
...
**Зачем: Проверяем MX домена, чтобы отправители могли определить сервер приема почты.**
...
**Зачем: Разрешите имя через системный NSS, то есть тем же путем, которым пользуются прикладные программы, а не только утилита dig.**
...
**Зачем: Свяжите порты SMTP, submission, IMAPS и HTTPS с процессами, затем убедитесь, что systemd не сообщает о failed units. Проверьте реальный socket, адрес, порт и процесс-владелец; состояние службы само по себе не доказывает наличие listener.**
...
**Зачем: Ищем аварийно завершившиеся units, чтобы не пропустить зависимую службу, влияющую на почтовую систему.**
...
**Зачем: Просматриваем эффективные нестандартные параметры Postfix, чтобы диагностировать установленную конфигурацию iRedMail, а не предполагать ее устройство.**
...
**Зачем: Просматриваем действующие настройки Dovecot, чтобы найти фактические пути и параметры, созданные установщиком iRedMail.**
...
**Зачем: Проверяем состояние SMTP- и IMAP-компонентов, чтобы обнаружить сбой службы до проверки почтового потока.**
...
**Зачем: Один сертификат может обслуживать несколько протоколов, но каждый endpoint проверяется своим способом запуска TLS и с SNI/именем сервера. Установите TLS-сеанс с SNI и проверкой имени; для SMTP STARTTLS явно указывается прикладной протокол.**
...
**Зачем: Проверяем цепочку доверия и имя сервера на IMAPS/993, чтобы почтовый клиент мог безопасно читать ящик.**
...
**Зачем: Проверяем сертификат HTTPS-входа webmail отдельно от SMTP и IMAP, поскольку веб-сервис может использовать другие TLS-файлы.**
...
**Зачем: До отправки подтвердите, что Postfix считает домен локальным/виртуальным, а Dovecot находит получателя и его INBOX. Выполните одну проверку почтового тракта: SMTP-ответ, queue ID, effective config или содержимое mailbox.**
...
**Зачем: Определяем источник сопоставлений виртуальных ящиков Postfix, чтобы диагностика опиралась на реальную схему iRedMail.**
...
**Зачем: Проверяем разрешение получателя в Dovecot, чтобы подтвердить существование виртуального пользователя и путь его почты.**
...
**Зачем: Проверяем доступность списка папок получателя до поиска тестового письма.**
...
**Зачем: Создаем уникальную тему тестового письма, чтобы найти именно эту отправку в ящике и не спутать ее с предыдущими тестами.**
...
**Зачем: Отправьте одно аутентифицированное тестовое письмо и сохраните SMTP-диалог: по нему далее извлекается queue ID и прослеживается доставка.**
...
**Зачем: Ищем подтверждения SMTP-аутентификации и постановки письма в очередь; принятие письма еще не доказывает доставку в ящик.**
...
**Зачем: Извлекаем идентификатор очереди из SMTP-сеанса, чтобы связать письмо с событиями Postfix в журнале.**
...
**Зачем: Останавливаем сценарий, если SMTP-сеанс не дал queue ID, чтобы дальнейший поиск не совпал с посторонними событиями.**
...
**Зачем: Выводим идентификаторы теста и очереди вместе для однозначного поиска одного письма на разных этапах доставки.**
...
**Зачем: Проверяем очередь после отправки, чтобы обнаружить отложенную доставку и сообщенную сервером причину.**
...
**Зачем: Используйте queue ID как корреляционный ключ: в журнале должны последовательно появиться прием сообщения и локальная доставка, после чего Dovecot найдет Subject в INBOX.**
...
**Зачем: Ищем письмо в INBOX по уникальному идентификатору теста, чтобы доказать фактическую доставку получателю.**
...
**Зачем: Повторно проверяем очередь после доставки, чтобы убедиться, что тестовое письмо не осталось в ожидании.**
...
**Зачем: Проверьте по HTTPS доступность страницы входа webmail. Факт доставки конкретного сообщения уже подтвержден предыдущим doveadm search; curl не выполняет пользовательский вход в mailbox.**
...
**Зачем: Проверяем признаки страницы webmail в ответе, чтобы отличить работающий интерфейс почты от стандартной страницы веб-сервера.**
...
**Зачем: Попытайтесь передать письмо между двумя внешними доменами без аутентификации и остановите SMTP-сеанс после RCPT. Исправный сервер обязан отклонить relaying.**
