...
**Зачем: Установите OpenVPN из package repository, затем уточните версию и шаблон unit: используются openvpn-server@.service или openvpn@.service. Установите пакеты только для выбранного семейства ОС. Ключ -y подтверждает пакетную транзакцию без дополнительного запроса.**
...
**Зачем: Устанавливаем OpenVPN из репозитория Debian/Ubuntu для создания сертификатного VPN-туннеля.**
...
**Зачем: Проверьте установленную ветку OpenVPN до применения конфигурации: работа рассчитана на синтаксис и поведение версии 2.6.**
...
**Зачем: Определяем доступные шаблоны units OpenVPN, чтобы запускать профиль с корректным именем службы в своей ОС.**
...
**Зачем: Создаем защищенный каталог серверной PKI, чтобы разместить сертификаты и ключи по путям полного OpenVPN-конфига.**
...
**Зачем: Устанавливаем доверенный CA и сертификат сервера в пути OpenVPN, чтобы сервер мог предъявить свою идентичность и проверять клиентов.**
...
**Зачем: Размещаем закрытый ключ сервера и общий tls-crypt key с доступом только root, защищая секреты VPN.**
...
**Зачем: topology subnet упрощает маршруты, tls-crypt защищает control channel, data-ciphers разрешает только AEAD. verify-client-cert требует client certificate.**
...
**Зачем: Запускаем серверный экземпляр OpenVPN и включаем его автозапуск для проверки доступности VPN после загрузки ВМ.**
...
**Зачем: Проверяем запуск нужного экземпляра OpenVPN, чтобы обнаружить ошибку конфигурации или загрузки ключей.**
...
**Зачем: Применяем сохраненные sysctl-настройки, чтобы IPv4 forwarding начал действовать без перезагрузки.**
...
**Зачем: Проверяем фактическое включение IPv4 forwarding, без которого сервер не сможет передавать трафик клиентов в LAN.**
...
**Зачем: Создайте permanent firewalld-правила только для учебных сетей, затем примените их одним reload после настройки zone и policy. Для UFW используйте отдельную альтернативную ветку ниже.**
...
**Зачем: Создаем отдельную зону для VPN-трафика, чтобы задать его доступ к LAN независимо от внешних подключений.**
...
**Зачем: Относим адресный пул VPN-клиентов к выделенной зоне, чтобы применять правила именно к трафику внутри туннеля.**
...
**Зачем: Создаем отдельную межзонную политику для VPN → LAN, чтобы управлять пересылкой независимо от доступа к самому серверу.**
...
**Зачем: Задаем VPN-зону как источник межзонной политики, ограничивая направление разрешенной пересылки.**
...
**Зачем: Задаем внутреннюю LAN-зону как назначение политики, чтобы разрешение не распространялось на все выходные сети.**
...
**Зачем: Разрешаем пересылку в выбранном направлении VPN → LAN; обратный маршрут и доступность узла проверяются отдельно.**
...
**Зачем: Применяем вместе подготовленные зоны и межзонную политику из постоянной конфигурации firewalld.**
...
**Зачем: Разрешаем установление OpenVPN-соединения по UDP/1194 только внешней учебной подсети в альтернативной ветке UFW.**
...
**Зачем: Разрешаем пересылку из туннеля в учебную LAN в ветке UFW, ограничивая интерфейсы и обе подсети.**
...
**Зачем: Добавляем обратный маршрут на учебном LAN-маршрутизаторе, чтобы ответы внутренних серверов возвращались в VPN-пул через VPN-сервер.**
...
**Зачем: На клиенте создайте отдельный закрытый каталог для доверенного CA, клиентского сертификата, закрытого ключа и общей копии tls-crypt key.**
...
**Зачем: Устанавливаем CA и клиентский сертификат, чтобы клиент мог проверить сервер и предъявить собственную идентичность.**
...
**Зачем: Закрытый ключ клиента и tls-crypt key доступны только root; их содержимое не выводится в терминал.**
...
**Зачем: remote-cert-tls и verify-x509-name не дают принять client certificate или сервер с другим именем. auth-nocache уменьшает время хранения credentials.**
...
**Зачем: Запускаем клиентский экземпляр с подготовленным профилем, чтобы установить защищенный туннель.**
...
**Зачем: Проверяем появление и адрес туннельного интерфейса, чтобы подтвердить создание сетевого устройства VPN.**
...
**Зачем: Проверяем выбор маршрута к внутреннему серверу, чтобы убедиться, что доступ в LAN направлен через туннель.**
...
**Зачем: Ищем подтверждения завершения инициализации и создания канала данных, сопоставляя их с интерфейсом и маршрутом.**
